...
class StreamingModelLoader:
    ...
    def __init__(
        ...
        self.key_translate = key_translate or (lambda k: k)
        self.dtype = dtype
    ...
    def _materialize_permanent_modules(self, model):
        ...
        _kt = self.key_translate
        def _pm_xlate(k):
            t = _kt(k)
            if t not in model_keys and ("model." + t) in model_keys:
                t = "model." + t
            return t
        keys = [k for k in all_keys if _pm_xlate(k) in model_keys]
    ...
    def _materialize_params(self, model, param_keys, device):
        ...
        # dense/shared MLP gate+up pairs fused into 2D gate_up_proj
        # (canonical Concatenate(dim=0), gate first)
        pair_pattern = re.compile(r"^(.*)\.(gate_proj|up_proj)\.weight$")
        pair_groups = defaultdict(dict)

        # ckpt->model key translation (e.g. native transformers renames
        # for VL checkpoints); generic fallback tries a 'model.' prefix.
        def _xlate(k):
            t = self.key_translate(k)
            if t not in model_keys and ("model." + t) in model_keys:
                t = "model." + t
            return t
        ...
        for key in param_keys:
            tkey = _xlate(key)
            m = expert_pattern.match(tkey)
            # pair check MUST precede the not-in-model skip: gate/up halves
            # never exist individually in the fused-native model.
            if not m:
                pm = pair_pattern.match(tkey)
                if pm and f"{pm.group(1)}.gate_up_proj.weight" in model_keys:
                    pair_groups[pm.group(1)][pm.group(2)] = key
                    continue
            if tkey not in model_keys and not m:
                continue
            if m:
                ...
                for key in keys:
                    if key in available:
                        ...
                        set_module_tensor_to_device(
                            model, _xlate(key), device, value=tensor
                        )

        # Fuse dense/shared gate+up pairs into 2D gate_up_proj (gate first).
        for prefix, parts in pair_groups.items():
            gk, uk = parts.get("gate_proj"), parts.get("up_proj")
            if not (gk and uk):
                continue
            tensors = {}
            for nm, ck in (("g", gk), ("u", uk)):
                with safe_open(
                    os.path.join(self.snapshot_dir, self.weight_map[ck]),
                    framework="pt", device="cpu",
                ) as fh:
                    tensors[nm] = fh.get_tensor(ck)
            fused = torch.cat([tensors["g"], tensors["u"]], dim=0)
            set_module_tensor_to_device(
                model, f"{prefix}.gate_up_proj.weight", device, value=fused
            )
            del tensors, fused
